...
# ──────────────────────────────────────────────────────────────
class AStarNode:
	var tile_pos : Vector2i  # The current node
	var g        : float     # Cost from the starting node to current node 
	var h        : float     # Manhattan Heuristic (educated guess) from current node to target
	var f        : float     # Total cost (g + h) - the algorithm follows the lower score
	var parent               # Traces back with the most optimal path
...
#  INSPECTOR SETTINGS
# ──────────────────────────────────────────────────────────────
@export var move_speed              : float = 155.0 # Dog's baseline speed (pixels per second)
@export var lookahead_k             : float = 5.0   # Predictive Targetting coefficient (looks n steps ahead)
@export var repath_interval         : float = 0.30  # Recalculates after every 0.4 seconds
@export var ema_alpha               : float = 0.35  # Exponential moving average (jitter filter)
@export var speed_boost_multiplier  : float = 1.40  # A 40% boost of speed 
@export var speed_boost_duration    : float = 2.00  # Duration of the speed boost ^^^
@export var streak_boost_multiplier : float = 1.65  # A 65% boost speed
@export var streak_boost_duration   : float = 1.50  # Duration of the streak boost ^^^
@export var panic_boost_multiplier  : float = 1.55  # A 55% boost
@export var panic_boost_duration    : float = 2.50  # Duration of the panic boost ^^^
@export var stuck_repath_limit      : int   = 3     # Stops dog from getting stuck in a path and repath
@export var proximity_tile_threshold : int = 3      # Lunges towards the player when proximity is within 2 tiles
...
#  PRIVATE STATE
# ──────────────────────────────────────────────────────────────
var _path                    : Array[Vector2] = []        # Takes the most optimal path (array of nodes)
...
var _sharp_turn_timer        : float = 0.0
...
# ──────────────────────────────────────────────────────────────
#  LIFECYCLE (Set up and dynamic speed)
# ──────────────────────────────────────────────────────────────

# Set up dog in the map
func _ready() -> void:
	add_to_group("dog")
	_chase_elapsed = 0.0
...
func _physics_process(delta: float) -> void:
	...
	_sharp_turn_timer   = max(0.0, _sharp_turn_timer  - delta)  
...
# Trigger speed boost upon rotation
func on_map_distortion() -> void:
	_trigger_speed_boost()
	if _repath_cooldown <= 0.0:
		_do_repath()

# Trigger speed boost when player repetitively sprints
...
# Trigger spped boost when player frantically changes direction
...
func _recalculate_path() -> void:
	...
	# EMA velocity smoothing (Player jitter smoothing)
	...
	# Surge detection (Trigger speed boost when player sprints)
	...
	# Confidence-scaled lookahead (Takes the Player's path confidence - 1:confident, 0:not confident) 
	...
	# Alongside / stuck guard (Walk toward player directly to get unstuck)
	...
	# Tile positions (Translating the grid)
...
func _run_astar(start: Vector2i, goal: Vector2i, iter_limit: int = _MAX_ITERATIONS) -> Array:
	var open_list  : Array      = []     # Unsearched nodes
	var closed_map : Dictionary = {}     # Searched nodes

	# Start at the current node
	open_list.append(AStarNode.new(start, 0.0, _manhattan(start, goal)))
	
	# Loop through the unsearched nodes
	...
	while not open_list.is_empty():
		...
		for d in [Vector2i(0, -1), Vector2i(0, 1), Vector2i(-1, 0), Vector2i(1, 0)]:
			...
			# Calculate tentative guess
			var tentative_g   : float     = current.g + 1.0
			var found         : bool      = false
			var existing_node : AStarNode

			# Check if the adjacent node was already searched
			...
			# Compares which path is shorter (existing or tentative guess?)
			if found:
				...
				# New node -> add to unsearched nodes
				open_list.append(
					AStarNode.new(nb_pos, tentative_g, _manhattan(nb_pos, goal), current))

	return []

# Trace step backwards when the player is reached
...
# Math helper for calculating the Manhattan Distance
...
# Predicts nearest walkable node if player is in a wall/non-walkable area
